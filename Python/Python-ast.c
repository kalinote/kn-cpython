...
static const char * const DoWhile_fields[]={
    "test",
    "body",
    "orelse",
};
...
init_types(struct ast_state *state)
{
    ...
    state->DoWhile_type = make_type(state, "DoWhile", state->stmt_type,
                                    DoWhile_fields, 3,
        "DoWhile(expr test, stmt* body, stmt* orelse)");
    if (!state->DoWhile_type) return 0;
...
stmt_ty
_PyAST_DoWhile(expr_ty test, asdl_stmt_seq * body, asdl_stmt_seq * orelse, int
               lineno, int col_offset, int end_lineno, int end_col_offset,
               PyArena *arena)
{
    stmt_ty p;
    if (!test) {
        PyErr_SetString(PyExc_ValueError,
                        "field 'test' is required for DoWhile");
        return NULL;
    }
    p = (stmt_ty)_PyArena_Malloc(arena, sizeof(*p));
    if (!p)
        return NULL;
    p->kind = DoWhile_kind;
    p->v.DoWhile.test = test;
    p->v.DoWhile.body = body;
    p->v.DoWhile.orelse = orelse;
    p->lineno = lineno;
    p->col_offset = col_offset;
    p->end_lineno = end_lineno;
    p->end_col_offset = end_col_offset;
...
ast2obj_stmt(struct ast_state *state, void* _o)
{
    ...
    case While_kind:
        ...
        break;
    case DoWhile_kind:
        tp = (PyTypeObject *)state->DoWhile_type;
        result = PyType_GenericNew(tp, NULL, NULL);
        if (!result) goto failed;
        value = ast2obj_expr(state, o->v.DoWhile.test);
        if (!value) goto failed;
        if (PyObject_SetAttr(result, state->test, value) == -1)
            goto failed;
        Py_DECREF(value);
        value = ast2obj_list(state, (asdl_seq*)o->v.DoWhile.body, ast2obj_stmt);
        if (!value) goto failed;
        if (PyObject_SetAttr(result, state->body, value) == -1)
            goto failed;
        Py_DECREF(value);
        value = ast2obj_list(state, (asdl_seq*)o->v.DoWhile.orelse,
                             ast2obj_stmt);
        if (!value) goto failed;
        if (PyObject_SetAttr(result, state->orelse, value) == -1)
            goto failed;
             ...
             arena)
{
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            PyErr_SetString(PyExc_TypeError, "required field \"test\" missing from DoWhile");
            return 1;
        }
        else {
            int res;
            if (_Py_EnterRecursiveCall(" while traversing 'DoWhile' node")) {
                goto failed;
            }
            res = obj2ast_expr(state, tmp, &test, arena);
            _Py_LeaveRecursiveCall();
        ...
        }
        {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "DoWhile field \"body\" must be a list, not a %.200s", _PyType_Name(Py_TYPE(tmp)));
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
            body = _Py_asdl_stmt_seq_new(len, arena);
            ...
            for (i = 0; i < len; i++) {
                ...
                if (res != 0) goto failed;
                if (len != PyList_GET_SIZE(tmp)) {
                    PyErr_SetString(PyExc_RuntimeError, "DoWhile field \"body\" changed size during iteration");
                    goto failed;
                }
                asdl_seq_SET(body, i, val);
            }
        ...
        }
        {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "DoWhile field \"orelse\" must be a list, not a %.200s", _PyType_Name(Py_TYPE(tmp)));
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
            orelse = _Py_asdl_stmt_seq_new(len, arena);
            ...
            for (i = 0; i < len; i++) {
                ...
                if (res != 0) goto failed;
                if (len != PyList_GET_SIZE(tmp)) {
                    PyErr_SetString(PyExc_RuntimeError, "DoWhile field \"orelse\" changed size during iteration");
                    goto failed;
                }
                asdl_seq_SET(orelse, i, val);
            }
        ...
        *out = _PyAST_DoWhile(test, body, orelse, lineno, col_offset,
                              end_lineno, end_col_offset, arena);
        if (*out == NULL) goto failed;
        return 0;
    }
...
astmodule_exec(PyObject *m)
{
    ...
    if (PyModule_AddObjectRef(m, "DoWhile", state->DoWhile_type) < 0) {
        return -1;
    }
